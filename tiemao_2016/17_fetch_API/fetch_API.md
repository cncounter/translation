...
AJAX饱受诟病的事莫过于其半遮半掩的底层API了.  **XMLHttpRequest** 并非是为如今的使用方式而设计的.  虽然各种框架对 **XHR** 的封装已经足够好用, 但其实我们可以做得更好。更好的方式是使用 `fetch` API。下面简单介绍下 `window.fetch` 方法, 在最新版的 Firefox 和 Chrome 中已经可以使用了。
