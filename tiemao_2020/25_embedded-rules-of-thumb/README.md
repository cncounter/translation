...
- 永远没有足够的时间来正确地实现，但总有足够的时间来完成。 （阿金法则）
- 估算日期而不是小时数可以保证项目的及时完成（Jack Ganssle）
  *“当开发人员不将日历时间与工程时间分开时，安排灾难是不可避免的。” （杰克·甘斯勒）
-“如果进度表显示人员利用率超过50％，则该项目将成比例落后。” （杰克·甘斯勒）
  *“一些数据表明，平均每个开发人员只有55％从事新产品工作。其他日常活动，从处理文书工作到谈论Survivor XVI，几乎消耗了一周的工作时间。” （杰克·甘斯勒）
-我们常常无法预见发展的困难领域
  *“我们对大多数项目的时间表估计有多糟糕，这令人惊讶吗？ 80％的嵌入式系统交付延迟。大多数专家认为，平均项目所花费的开发工作是最初预算的两倍。” （杰克·甘斯勒）
-5％的功能消耗了80％的调试时间（Jack Ganssle）
  *“我观察到，大多数项目都陷入了调试周期，通常占整个进度的一半。显然，如果我们能够对代表我们大多数麻烦的少数几个功能做些事情，那么该项目将尽快出台。” （杰克·甘斯勒）
-时间线的增长速度远远超过固件大小-代码行数增加了一倍，交货日期增加了两倍以上（Barry Boehm）
-“代码的前90％占了开发时间的前90％。剩下的10％的代码占了开发时间的90％。” （汤姆·嘉吉）
-在将旧代码移植到新项目时，如果修改超过25％，那么进度安排就不会增加（Richard Selby）
-加载了90％处理器能力的系统所需的开发时间是加载70％或更少的系统的两倍。 95％的加载使开发时间增加了三倍。 （杰克·甘斯勒）
  *“当只剩下几个字节时，即使微不足道的功能也可能需要数周时间，因为开发人员必须重写大量代码以释放内存或CPU周期。” （杰克·甘斯勒）
-您制定的时间表似乎是虚构的完整作品，直到客户因不满足而解雇您为止。 （阿金法则）
-有时，到达终点的最快方法是丢掉所有东西并重新开始。 （阿金法则）
-（帕顿的计划规划法）现在猛烈执行的好的计划比下周的完善计划要好。 （阿金法则）
