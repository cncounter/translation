...
如果表中的数据量很大，聚簇索引结构可以减少磁盘I/O操作，相比而言, 将索引和数据放到不同的磁盘页面则会需要多次磁盘IO。
...
除了聚集索引, 其他索引都称为 [二级索引](https://dev.mysql.com/doc/refman/5.7/en/glossary.html#glos_secondary_index), 也可以称为普通索引。
在 InnoDB 中，二级索引上的每条记录, 都包含创建索引时指定的列, 以及该行的主键列(值)。
查询时, `InnoDB` 再通过这个主键值去定位聚集索引中的行。

If the primary key is long, the secondary indexes use more space, so it is advantageous to have a short primary key.

如果主键很长，二级索引会占用更多的空间，这时候主键使用占用空间较小的数据类型是有利的。
...
有关 `InnoDB` 聚集索引和二级索引的利用指南，请参阅 [Section 8.3, `Optimization and Indexes`](https://dev.mysql.com/doc/refman/5.7/en/optimization-indexes.html).
