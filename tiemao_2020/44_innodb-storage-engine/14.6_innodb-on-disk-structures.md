...
与系统表空间类似，截断或删除存储在通用表空间中的表, 会在通用表空间 [.ibd data file](https://dev.mysql.com/doc/refman/5.7/en/glossary.html#glos_ibd_file) 中产生空闲区域, 而个文件又只能用于新的 `InnoDB` 数据。 在 [`DROP TABLE`](https://dev.mysql.com/doc/refman/5.7/en/drop-table.html) 操作期间删除 file-per-table 表空间，对应的磁盘空间不会返还给操作系统。
