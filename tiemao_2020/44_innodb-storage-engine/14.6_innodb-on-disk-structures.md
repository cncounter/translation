...
#### 14.6.1.1 创建使用InnoDB存储引擎的表


To create an `InnoDB` table, use the [`CREATE TABLE`](https://dev.mysql.com/doc/refman/5.7/en/create-table.html) statement.

要创建一个 `InnoDB` 表，请使用 [`CREATE TABLE`](https://dev.mysql.com/doc/refman/5.7/en/create-table.html) 语句。
...
如果 `InnoDB` 是默认存储引擎的话，则无需指定 `ENGINE=InnoDB` 子句。 要查看默认的存储引擎，请执行以下语句：
...
使用InnoDB创建表时，MySQL会在 data 目录下的数据库目录中创建一个 [.frm 文件](https://dev.mysql.com/doc/refman/5.7/en/glossary.html#glos_frm_file)。 有关`.frm`文件的更多信息，请参见[InnoDB Tables and .frm Files](https://dev.mysql.com/doc/refman/5.7/en/using-innodb-tables.html#innodb-frm-file)。

对于在 file-per-table 表空间中创建的表，MySQL还默认在数据库目录下创建一个 [.ibd](https://dev.mysql.com/doc/refman/5.7/en/glossary.html#glos_ibd_file) tablespace 文件。

在 `InnoDB` system tablespace 中创建的表, 则是放到 MySQL data 目录下已有的 [ibdata文件](https://dev.mysql.com/doc/refman/5.7/en/glossary.html#glos_ibd_file)中。

在 general tablespace 中创建的表，位于常规表空间的现有 [.ibd 文件](https://dev.mysql.com/doc/refman/5.7/en/glossary.html#glos_ibd_file)中。 常规表空间文件, 可以在MySQL数据目录内部或外部创建。 更多信息，请参见 [Section 14.6.3.3, “General Tablespaces”](https://dev.mysql.com/doc/refman/5.7/en/general-tablespaces.html)。

在内部，InnoDB将每个表的条目添加到InnoDB数据字典中。 该条目包括数据库名称。 例如，如果在 `test` 数据库中创建了表`t1`，则数据字典条目中的数据库名称为 `'test/t1'`。 这意味着我们可以在其他数据库中创建相同名称的表(`t1`)，并且表名不会在`InnoDB`内部冲突。
...
- [Using the DATA DIRECTORY Clause](https://dev.mysql.com/doc/refman/5.7/en/innodb-create-table-external.html#innodb-create-table-external-data-directory)
- [Using CREATE TABLE ... TABLESPACE Syntax](https://dev.mysql.com/doc/refman/5.7/en/innodb-create-table-external.html#innodb-create-table-external-tablespace-syntax)
- [Creating a Table in an External General Tablespace](https://dev.mysql.com/doc/refman/5.7/en/innodb-create-table-external.html#innodb-create-table-external-tablespace)
...
- [Prerequisites](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html#innodb-table-import-prerequsites)
- [Importing Tables](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html#innodb-table-import-example)
- [Importing Partitioned Tables](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html#innodb-table-import-partitioned-table)
- [Importing Table Partitions](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html#innodb-table-import-partitions)
- [Limitations](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html#innodb-table-import-limitations)
- [Usage Notes](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html#innodb-table-import-usage-notes)
- [Internals](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html#innodb-table-import-internals)
...
A table that resides in a file-per-table tablespace can be imported from another MySQL server instance or from a backup using the *Transportable Tablespace* feature. See [Section 14.6.1.3, “Importing InnoDB Tables”](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html).
...
You can use the `DATA DIRECTORY` clause of the [`CREATE TABLE`](https://dev.mysql.com/doc/refman/5.7/en/create-table.html) statement to implicitly create a file-per-table tablespace data file outside of the data directory. For more information, see [Section 14.6.1.2, “Creating Tables Externally”](https://dev.mysql.com/doc/refman/5.7/en/innodb-create-table-external.html).
...
- File-per-table tablespace data files can be created on separate storage devices for I/O optimization, space management, or backup purposes. See [Section 14.6.1.2, “Creating Tables Externally”](https://dev.mysql.com/doc/refman/5.7/en/innodb-create-table-external.html).
- You can import a table that resides in a file-per-table tablespace from another MySQL instance. See [Section 14.6.1.3, “Importing InnoDB Tables”](https://dev.mysql.com/doc/refman/5.7/en/innodb-table-import.html).
