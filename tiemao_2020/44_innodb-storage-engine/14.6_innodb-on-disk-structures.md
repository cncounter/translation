...
- 必须启用 [`innodb_file_per_table`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_file_per_table) 变量，默认情况下。
- 表空间的页面大小必须与目标 MySQL 服务器实例的页面大小匹配。 `InnoDB` 页面大小由 [`innodb_page_size`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_page_size) 变量定义，该变量在初始化 MySQL 时配置服务器实例。
- 如果表是外键关系，执行前必须禁用[`foreign_key_checks`](https://dev.mysql.com/doc/refman/5.7/en/server-system-variables.html#sysvar_foreign_key_checks) `丢弃表空间`。此外，您应该在相同的逻辑时间点导出所有与外键相关的表，如 [`ALTER TABLE ... IMPORT TABLESPACE`](https://dev.mysql.com/doc/refman/5.7/en/alter -table.html) 不对导入的数据强制执行外键约束。为此，请停止更新相关表，提交所有事务，获取表上的共享锁，并执行导出操作。
- 从另一个 MySQL 服务器实例导入表时，两个 MySQL 服务器实例必须具有通用可用性 (GA) 状态并且必须是相同的版本。否则，必须在导入它的同一个 MySQL 服务器实例上创建该表。
- 如果表是通过在[`CREATE TABLE`](https://dev.mysql.com/doc/refman/5.7/en/create-table.html)中指定`DATA DIRECTORY`子句在外部目录中创建的) 语句，您在目标实例上替换的表必须使用相同的 `DATA DIRECTORY` 子句定义。如果子句不匹配，则报告模式不匹配错误。要确定源表是否使用 `DATA DIRECTORY` 子句定义，请使用 [`SHOW CREATE TABLE`](https://dev.mysql.com/doc/refman/5.7/en/show-create-table.html ) 查看表定义。有关使用 `DATA DIRECTORY` 子句的信息，请参阅 [第 14.6.1.2 节，“在外部创建表”](#14.6.1.2)。
- 如果在表定义中没有明确定义 `ROW_FORMAT` 选项或使用 `ROW_FORMAT=DEFAULT`，则 [`innodb_default_row_format`](https://dev.mysql.com/doc/refman/5.7/en/innodb -parameters.html#sysvar_innodb_default_row_format) 设置在源和目标实例上必须相同。否则，尝试导入操作时会报告架构不匹配错误。使用 [`SHOW CREATE TABLE`](https://dev.mysql.com/doc/refman/5.7/en/show-create-table.html) 检查表定义。使用 [`SHOW VARIABLES`](https://dev.mysql.com/doc/refman/5.7/en/show-variables.html) 检查 [`innodb_default_row_format`](https://dev.mysql.com /doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_default_row_format) 设置。相关信息参见【定义表的行格式】(https://dev.mysql.com/doc/refman/5.7/en/innodb-row-format.html#innodb-row-format-defining)。
...
- 1. 在目标实例上，创建一个与您要导入的表具有相同定义的表。 （您可以使用 [`SHOW CREATE TABLE`](https://dev.mysql.com/doc/refman/5.7/en/show-create-table.html) 语法获取表定义。）如果表定义没有 不匹配，当您尝试导入操作时会报告架构不匹配错误。
...
- 2. 在目标实例上，丢弃您刚刚创建的表的表空间。 （导入前必须丢弃接收表的表空间。）
...
3. 在源实例上，运行 [`FLUSH TABLES ... FOR EXPORT`](https://dev.mysql.com/doc/refman/5.7/en/flush.html#flush-tables-for-export- with-list) 以停止要导入的表。 当一个表被停顿时，该表只允许只读事务。
...
[`FLUSH TABLES ... FOR EXPORT`](https://dev.mysql.com/doc/refman/5.7/en/flush.html#flush-tables-for-export-with-list) 确保更改为 命名表已刷新到磁盘，以便可以在服务器运行时制作二进制表副本。 当 [`FLUSH TABLES ... FOR EXPORT`](https://dev.mysql.com/doc/refman/5.7/en/flush.html#flush-tables-for-export-with-list) 运行时， `InnoDB` 在表的模式目录中生成一个 `.cfg` 元数据文件。 `.cfg` 文件包含在导入操作期间用于模式验证的元数据。
...
在释放共享锁之前，必须复制 `.ibd` 文件和 `.cfg` 文件，如下一步所述。

> 笔记
>
> 如果您从加密表空间导入表，`InnoDB` 会生成一个 `.cfp` 文件以及一个 `.cfg` 元数据文件。 `.cfp` 文件必须与 `.cfg` 文件一起复制到目标实例。 `.cfp` 文件包含一个传输密钥和一个加密的表空间密钥。 在导入时，`InnoDB` 使用传输密钥来解密表空间密钥。 有关相关信息，请参阅 [第 14.14 节，“InnoDB Data-at-Rest 加密”](https://dev.mysql.com/doc/refman/5.7/en/innodb-data-encryption.html)。
...
5.在源实例上，使用[`UNLOCK TABLES`](https://dev.mysql.com/doc/refman/5.7/en/lock-tables.html)释放[`FLUSH TABLES ... FOR EXPORT`](https://dev.mysql.com/doc/refman/5.7/en/flush.html#flush-tables-for-export-with-list) 语句：
