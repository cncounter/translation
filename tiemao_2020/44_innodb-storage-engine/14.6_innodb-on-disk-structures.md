...
创建InnoDB表时，MySQL会在data目录下面的数据库目录中创建一个 [`.frm` 文件](https://dev.mysql.com/doc/refman/5.7/en/glossary.html#glos_frm_file)。 有关 `.frm` 文件的更多信息，请参见[InnoDB Tables and .frm Files](https://dev.mysql.com/doc/refman/5.7/en/using-innodb-tables.html#innodb-frm-file)。
