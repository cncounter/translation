...
一种方便的方法是在创建任何数据库或表之前，将以下配置添加到 `my.cnf` or `my.ini` 文件的 `[mysqld]` 节:
...
2. 执行下面的 [`ALTER TABLE`](https://dev.mysql.com/doc/refman/5.7/en/alter-table.html) 语句来删除当前的 `.ibd` 文件:
...
4. 执行下面的 [`ALTER TABLE`](https://dev.mysql.com/doc/refman/5.7/en/alter-table.html) 语句, 告诉 `InnoDB`  对这个表使用新的 `.ibd` 文件:
...
#### 14.6.1.6  InnoDB 的 AUTO_INCREMENT 处理机制
...
要在`InnoDB`表中使用 `AUTO_INCREMENT` 机制，必须将 `AUTO_INCREMENT` 列指定为索引的一部分，以便执行等价于 `SELECT MAX(auto_incr_column)` 的索引查询语句, 在表中查找该列的最大值。
通常，这是通过将该列作为某个索引的第一列来实现的。

本节描述了 `AUTO_INCREMENT` 锁模式的行为、 使用不同 `AUTO_INCREMENT` 锁模式设置的含义、 以及`InnoDB` 如何初始化 `AUTO_INCREMENT` 计数器。
...
本节描述用于生成自动递增值的`AUTO_INCREMENT`锁模式的行为，以及每种锁模式如何影响复制。 使用 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 配置参数在启动时配置自动增量锁定模式。

以下术语用于描述 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 设置：
...
- `[`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html)-like`语句

在表中生成新行的所有语句，包括 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html)、[`INSERT ... SELECT`]( https://dev.mysql.com/doc/refman/5.7/en/insert-select.html), [`REPLACE`](https://dev.mysql.com/doc/refman/5.7/en/replace .html)、[`REPLACE ... SELECT`](https://dev.mysql.com/doc/refman/5.7/en/replace.html) 和 [`LOAD DATA`](https://dev .mysql.com/doc/refman/5.7/en/load-data.html）。包括`简单插入`、`批量插入`和`混合模式`插入。
...
可以预先确定要插入的行数的语句（在最初处理语句时）。这包括单行和多行 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 和 [`REPLACE`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) mysql.com/doc/refman/5.7/en/replace.html) 没有嵌套子查询的语句，但没有 [`INSERT ... ON DUPLICATE KEY UPDATE`](https://dev.mysql.com/ doc/refman/5.7/en/insert-on-duplicate.html）。
...
要插入的行数（以及所需的自动增量值的数量）事先未知的语句。 这包括 [`INSERT ... SELECT`](https://dev.mysql.com/doc/refman/5.7/en/insert-select.html), [`REPLACE ... SELECT`](https://dev.mysql.com/doc/refman/5.7/en/insert-select.html) /dev.mysql.com/doc/refman/5.7/en/replace.html) 和 [`LOAD DATA`](https://dev.mysql.com/doc/refman/5.7/en/load-data. html) 语句，但不是简单的`插入`。 `InnoDB` 在处理每一行时一次为 `AUTO_INCREMENT` 列分配一个新值。
...
这些是`简单插入`语句，为某些（但不是全部）新行指定自动增量值。 下面是一个示例，其中 `c1` 是表 `t1` 的一个 `AUTO_INCREMENT` 列：
...
另一种类型的`混合模式插入`是 [`INSERT ... ON DUPLICATE KEY UPDATE`](https://dev.mysql.com/doc/refman/5.7/en/insert-on-duplicate.html)， 在最坏的情况下，实际上是一个 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 后跟一个 [`UPDATE`](https:// dev.mysql.com/doc/refman/5.7/en/update.html)，其中在更新阶段可能会或可能不会使用为`AUTO_INCREMENT`列分配的值。
...
[`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 配置参数有三种可能的设置。 设置为 0、1 或 2，分别用于`传统`、`连续`或`交错`锁定模式。
...
传统的锁定模式提供了与 MySQL 中引入 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 配置参数之前存在的相同行为5.1.提供传统的锁定模式选项是为了向后兼容、性能测试和解决`混合模式插入`的问题，因为语义可能存在差异。

在这种锁定模式下，所有`INSERT-like`语句都会获得一个特殊的表级`AUTO-INC`锁，用于插入到带有`AUTO_INCREMENT`列的表中。此锁通常保持在语句的末尾（而不是事务的末尾），以确保为给定的 [`INSERT`] 序列以可预测和可重复的顺序分配自动增量值（https:// dev.mysql.com/doc/refman/5.7/en/insert.html) 语句，并确保任何给定语句分配的自动增量值是连续的。

在基于语句的复制的情况下，这意味着在副本服务器上复制 SQL 语句时，自动增量列使用的值与源服务器上的值相同。多个 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句的执行结果是确定性的，并且副本再现与源上相同的数据。如果多个[`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html)语句生成的自增值被交错，则两个并发[`INSERT`]的结果(https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句将是不确定的，并且不能使用基于语句的复制可靠地传播到副本服务器。
...
假设有两个事务在运行，每个事务都将行插入到一个带有`AUTO_INCREMENT`列的表中。 一个事务使用插入 1000 行的 [`INSERT ... SELECT`](https://dev.mysql.com/doc/refman/5.7/en/insert-select.html) 语句，另一个使用 插入一行的简单 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句：
...
`InnoDB` 无法预先知道从 [`INSERT`]( https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句在 Tx1 中，并随着语句的进行一次分配一个自动增量值。使用表级锁，一直保持到语句末尾，只有一个 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句引用表` t1`可以一次执行，不同语句生成自增数不交错。 Tx1 [`INSERT ... SELECT`](https://dev.mysql.com/doc/refman/5.7/en/insert-select.html)语句生成的自增值是连续的，并且( Tx2 中的 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句使用的单个）自动增量值要么小于要么大于所有使用的值对于 Tx1，取决于首先执行的语句。

只要 SQL 语句在从二进制日志重放时（使用基于语句的复制时，或在恢复场景中）以相同的顺序执行，结果与 Tx1 和 Tx2 首次运行时的结果相同。因此，表级锁一直保持到语句结束使 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句使用自动增量安全使用使用基于语句的复制。但是，当多个事务同时执行插入语句时，这些表级锁会限制并发性和可伸缩性。

在前面的例子中，如果没有表级锁，用于[`INSERT`]的自增列的值(https://dev.mysql.com/doc/refman/5.7/en/insert .html) 在 Tx2 中精确取决于语句执行的时间。如果 Tx2 的 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 执行而 [`INSERT`](https://dev.mysql.com /doc/refman/5.7/en/insert.html) 的 Tx1 正在运行（而不是在它开始之前或完成之后），两个 [`INSERT`](https://dev .mysql.com/doc/refman/5.7/en/insert.html) 语句是不确定的，并且可能因运行而异。
...
如果您不使用二进制日志来重放 SQL 语句作为恢复或复制的一部分，[交错](https://dev.mysql.com/doc/refman/5.7/en/innodb-auto-increment-handling. html#innodb-auto-increment-lock-mode-interleaved) 锁模式可用于消除所有表级`AUTO-INC`锁的使用，以获得更高的并发性和性能，代价是允许自动递增中的间隙 由语句分配的编号，并且可能将同时执行的语句分配的编号交错。
...
这是默认的锁定模式。在这种模式下，`批量插入`使用特殊的`AUTO-INC`表级锁并将其保持到语句结束。这适用于所有 [`INSERT ... SELECT`](https://dev.mysql.com/doc/refman/5.7/en/insert-select.html), [`REPLACE ... SELECT`](https://dev.mysql.com/doc/refman/5.7/en/insert-select.html) ://dev.mysql.com/doc/refman/5.7/en/replace.html) 和 [`LOAD DATA`](https://dev.mysql.com/doc/refman/5.7/en/load- data.html) 语句。一次只能执行一个持有`AUTO-INC`锁的语句。如果批量插入操作的源表与目标表不同，则在对源表中选择的第一行采取共享锁后，对目标表采取`AUTO-INC`锁。如果批量插入操作的源和目标是同一个表，则在对所有选定行采取共享锁后，将采取`AUTO-INC` 锁。

`简单插入`（预先知道要插入的行数）通过在互斥锁（一种轻量级的锁）只在分配过程中保持，*不*直到语句完成。除非另一个事务持有`AUTO-INC`锁，否则不使用表级`AUTO-INC`锁。如果另一个事务持有`AUTO-INC`锁，则`简单插入`等待`AUTO-INC`锁，就好像它是`批量插入`一样。
...
这种锁定模式确保，在存在 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句的情况下，行数事先未知（并且随着语句的进行分配自动增量编号），由任何`[`INSERT`]（https://dev.mysql.com/doc/refman/5.7/en/insert.html）分配的所有自动增量值)-like`语句是连续的，并且操作对于基于语句的复制是安全的。

简而言之，这种锁定模式显着提高了可扩展性，同时可以安全地用于基于语句的复制。此外，与`传统`锁定模式一样，任何给定语句分配的自动递增编号都是*连续的*。对于任何使用自动增量的语句，与`传统`模式相比，语义上*没有变化*，只有一个重要的例外。

例外是`混合模式插入`，在这种情况下，用户为多行`简单插入`中的某些（但不是全部）行的`AUTO_INCREMENT`列提供显式值。对于此类插入，`InnoDB` 分配比要插入的行数更多的自动增量值。但是，自动分配的所有值都是连续生成的（因此高于）由最近执行的先前语句生成的自动增量值。 `多余`的数字丢失了。
...
在这种锁模式下，没有`[`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html)-like`语句使用表级`AUTO-INC`锁, 多个语句可以同时执行。这是最快和最具扩展性的锁定模式，但在使用基于语句的复制或恢复场景时，当从二进制日志重放 SQL 语句时，它*不安全*。

在这种锁定模式下，自动递增值保证在所有并发执行的`[`INSERT`]（https://dev.mysql.com/doc/refman/5.7/en/insert.html）中唯一且单调递增-like`的说法。但是，由于多个语句可以同时生成数字（即，数字的分配是*交错* 跨语句的），因此为任何给定语句插入的行生成的值可能不是连续的。
...
如果您使用基于语句的复制，请将 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 设置为 0 或 1 并使用相同的 源及其副本上的值。 如果您使用 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html)，则不能确保副本上的自动增量值与源上的相同 #sysvar_innodb_autoinc_lock_mode) = 2（`交错`）或源和副本不使用相同锁定模式的配置。

如果您使用基于行或混合格式的复制，所有自增锁模式都是安全的，因为基于行的复制对 SQL 语句的执行顺序不敏感（混合格式使用基于行的复制） 复制对基于语句的复制不安全的任何语句）。
...
在所有锁定模式（0、1 和 2）中，如果生成自增值的事务回滚，则这些自增值将`丢失`。 一旦为自增列生成了值，就不能回滚，无论`[`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html )-like`语句完成，包含的事务是否回滚。 此类丢失的值不会重用。 因此，存储在表的`AUTO_INCREMENT`列中的值可能存在间隙。
...
在所有锁定模式（0、1 和 2）中，如果用户在 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/) 中为 `AUTO_INCREMENT` 列指定 NULL 或 0 en/insert.html)，`InnoDB` 将该行视为未指定该值并为其生成一个新值。
...
在所有锁定模式（0、1 和 2）中，如果为 `AUTO_INCREMENT` 列分配负值，则不会定义自动递增机制的行为。
...
- 如果`AUTO_INCREMENT` 值大于指定整数类型的最大整数

在所有锁定模式（0、1 和 2）中，如果值变得大于可以存储在指定整数类型中的最大整数，则不会定义自动递增机制的行为。
...
将 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 设置为 0（`传统`）或 1（`连续`），任何给定语句生成的自增值都是连续的，没有间隙，因为表级`AUTO-INC`锁一直保持到语句结束，并且一次只能执行一个这样的语句。

当 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 设置为 2（`交错`）时，自动`批量插入`生成的增量值，但前提是同时执行`[`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html)-like`语句。
...
考虑`混合模式插入`，其中`简单插入`为某些（但不是全部）结果行指定自动增量值。 这样的语句在锁定模式 0、1 和 2 中的行为不同。例如，假设 `c1` 是表 `t1` 的一个 `AUTO_INCREMENT` 列，并且最近自动生成的序列号是 100。
...
将 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 设置为 0（`传统`），四个新行是：
...
下一个可用的自动增量值为 103，因为自动增量值一次分配一个，而不是在语句执行开始时一次分配。 无论是否同时执行`[`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html)-like`语句（任何类型），此结果都是正确的。

将 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 设置为 1（`连续`），四个新行也是：
...
但是，在这种情况下，下一个可用的自增值是 105，而不是 103，因为在处理语句时分配了四个自增值，但只使用了两个。 无论是否同时执行`[`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html)-like`语句（任何类型），此结果都是正确的。

将 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 设置为模式 2（`交错`），四个新行是：
...
`x` 和 `y` 的值是唯一的并且比之前生成的任何行都大。 但是，`x` 和`y` 的具体值取决于并发执行语句生成的自增值的数量。
...
使用任何 [`innodb_autoinc_lock_mode`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_autoinc_lock_mode) 设置，此语句会生成重复密钥错误 23000（`Can't 写；表中的重复键`) 因为为行`(NULL, 'b')` 分配了101，并且行`(101, 'c')` 的插入失败。
...
- 在 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 语句序列中间修改 `AUTO_INCREMENT` 列值

在所有锁定模式（0、1和2）下，修改[`INSERT`]序列中间的`AUTO_INCREMENT`列值（https://dev.mysql.com/doc/refman/5.7/en /insert.html) 语句可能会导致`重复条目`错误。 例如，如果您执行 [`UPDATE`](https://dev.mysql.com/doc/refman/5.7/en/update.html) 操作将 `AUTO_INCREMENT` 列值更改为大于 当前最大自动增量值，未指定未使用的自动增量值的后续 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 操作可能会遇到` 重复输入`错误。 下面的示例演示了此行为。
...
如果为`InnoDB`表指定`AUTO_INCREMENT`列，则`InnoDB`数据字典中的表句柄包含一个称为自动增量计数器的特殊计数器，用于为该列分配新值。 该计数器仅存储在主内存中，而不存储在磁盘上。

为了在服务器重新启动后初始化自动递增计数器，`InnoDB` 在第一次插入包含 `AUTO_INCREMENT` 列的表时执行以下等效语句。
...
`InnoDB` 递增语句检索到的值，并将其分配给列和表的自动递增计数器。默认情况下，该值递增 1。此默认值可以被 [`auto_increment_increment`](https://dev.mysql.com/doc/refman/5.7/en/replication-options-source.html#sysvar_auto_increment_increment 覆盖) 配置设置。

如果表为空，`InnoDB` 使用值`1`。这个默认值可以被 [`auto_increment_offset`](https://dev.mysql.com/doc/refman/5.7/en/replication-options-source.html#sysvar_auto_increment_offset) 配置设置覆盖。

如果 [`SHOW TABLE STATUS`](https://dev.mysql.com/doc/refman/5.7/en/show-table-status.html) 语句在自动递增计数器初始化之前检查表，` InnoDB` 初始化但不增加值。该值被存储以供以后插入使用。此初始化使用对表的正常排他锁定读取，并且锁定持续到事务结束。 `InnoDB` 遵循相同的过程来为新创建的表初始化自动递增计数器。
...
在初始化自动递增计数器后，如果您没有为`AUTO_INCREMENT`列明确指定值，`InnoDB`会递增计数器并将新值分配给该列。如果插入显式指定列值的行，并且该值大于当前计数器值，则将计数器设置为指定的列值。

只要服务器运行，`InnoDB` 就会使用内存中的自动递增计数器。当服务器停止并重新启动时，`InnoDB` 为第一个 [`INSERT`](https://dev.mysql.com/doc/refman/5.7/en/insert.html) 重新初始化每个表的计数器到表，如前所述。

服务器重启也会取消 [`CREATE TABLE`] 中 `AUTO_INCREMENT = `N` 表选项的效果(https://dev.mysql.com/doc/refman/5.7/en/create-table.html)和 [`ALTER TABLE`](https://dev.mysql.com/doc/refman/5.7/en/alter-table.html) 语句，您可以将它们与 `InnoDB` 表一起使用来设置初始计数器值或改变当前的计数器值。
...
- 当 `AUTO_INCREMENT` 整数列用完值时，后续的 `INSERT` 操作返回重复键错误。 这是 MySQL 的一般行为。
