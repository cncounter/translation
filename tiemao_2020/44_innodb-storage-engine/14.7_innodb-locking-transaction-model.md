...
即使在同一事务中, 每次一致性读都会设置和读取自己的新快照。 有关一致性读的信息, 请参见[Section 14.7.2.3, “Consistent Nonlocking Reads”](#14.7.2.3)。
...
使用 `LOCK IN SHARE MODE` 时, 将使用锁定读: `SELECT`可能会被阻塞, 直到包含最新行的事务结束为止。 请参考[Section 14.7.2.4, “Locking Reads”](#14.7.2.4)。
...
不管语句中是否有`WHERE`条件都会上锁。`InnoDB`并不会记录确切的WHERE条件, 只知道扫描到了哪些索引范围。 一般使用的是 临键锁(`next-key lock`), 这样就可以阻塞(block)其他事务将新行插入到前面的间隙(gap)中。
...
如果在搜索中使用了二级索引, 并且要设置的是排他锁, 则`InnoDB`还会检索相应的聚簇索引记录(clustered index record)并对其上锁。
...
- 当 `UPDATE` 修改聚簇索引记录时，将对受影响的二级索引记录进行隐式锁定。 在插入新的二级索引记录之前, 执行重复项检查扫描时，以及在插入新的二级索引记录时， `UPDATE` 操作还会对受影响的二级索引记录设置共享锁。
...
- `INSERT INTO T SELECT ... FROM S WHERE ...` 在插入`T`的每一行上设置排他索引记录锁（不带间隙锁）。 如果事务隔离级别为`READ COMMITTED`，或者启用了 `innodb_locks_unsafe_for_binlog` 但是事务隔离级别不是 `SERIALIZABLE`，则InnoDB会对S表进行搜索，以使其保持一致性读（无锁）。 其他情况下，`InnoDB` 会在 `S` 的行上设置共享临键锁。 为什么后一种情况下InnoDB必须设置锁呢? 在使用基于语句的 bin-log 进行前滚恢复时，必须以和原始操作完全相同的方式来执行每个SQL语句。
  ...
  当在 `REPLACE INTO t SELECT ... FROM s WHERE ...` 或者 `UPDATE t ... WHERE col IN (SELECT ... FROM s ...)` 构造中使用 `SELECT` 时，`InnoDB`在 `s` 表的行上设置共享的临键锁。
...
- `LOCK TABLES` 会设置表锁，但设置这些锁的是比 `InnoDB` 更高层级的 MySQL 锁。 如果 `innodb_table_locks = 1`（默认值）并且 `autocommit = 0`，则 InnoDB之 会感知到表锁，而 MySQL 层级也会感知到行级锁。
   ...
   否则，InnoDB的自动死锁检测将无法检测到涉及这类表锁的死锁情况。 同样，由于在这种情况下，较高的MySQL层级也不知道行级锁，因此可能会到被其他会话持有行级锁的表上去获取表锁。 但是，这并不危及事务完整性，如 [Section 14.7.5.2, “Deadlock Detection”](#14.7.5.2) 所述。
...
- 提交或中止事务时，将释放事务持有的所有 `InnoDB` 锁。 所以在 `autocommit=1` 模式下, 在`InnoDB`表上执行 `LOCK TABLES` 并没有什么意义，因为获取的 `InnoDB` 表锁会立即释放。
...
为了防止产生幻象, InnoDB使用一种称为"临键锁定"的算法, 该算法结合了索引行锁定和间隙锁定。 InnoDB执行行级锁定的方式是, 当它搜索或扫描表索引时, 会在遇到的索引记录上设置共享或互斥锁。因此, 行级锁实际上是索引记录锁。此外, 索引记录上的临键锁定也会影响该索引记录之前的"间隙"。即, 临键锁定是索引记录锁定加上索引记录之前的间隙上的间隙锁定。如果一个会话在索引中的记录" R"上具有共享或排他锁, 则另一会话不能在索引顺序中紧接在" R"之前的间隙中插入新的索引记录。
...
您可以使用临键锁定在应用程序中实施唯一性检查：如果您以共享模式读取数据，但没有看到要插入的行的重复项，则可以安全地插入行并知道在读取期间在行的后继上设置的next键锁可以防止任何人同时为行插入重复项。因此，临键锁定使您可以“锁定”表中不存在的内容。
