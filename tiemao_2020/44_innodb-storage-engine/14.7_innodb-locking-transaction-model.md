...
假设使用默认的 `REPEATABLE READ` 隔离级别。 在我们执行普通的 `SELECT` 一致读取时，`InnoDB` 会为当前事务指定一个时间点，事务中的所有查询都根据这个时间点来确定可以看到哪些数据。 如果另一个事务在给定时间点后删除了一行数据并提交， 那么我们的事务则看不到该行已被删除。插入和更新的处理方式也是一样的。
...
数据库状态的快照适用于事务中的“ SELECT”语句，而不一定适用于DML语句。 如果您插入或修改一些行然后提交该事务，则从另一个并发的“ REPEATABLE READ”事务发出的“ DELETE”或“ UPDATE”语句可能会影响那些刚刚提交的行，即使会话无法查询它们。 如果某个事务确实更新或删除了另一个事务提交的行，则这些更改对于当前事务而言确实可见。 例如，您可能会遇到以下情况：
...
您可以通过提交事务，然后执行另一个“选择”或“使用一致的快照开始交易”来提前时间。
...
如果要查看数据库的“最原始”状态，请使用“ READ COMMITTED”隔离级别或锁定读取：
...
使用“ READ COMMITTED”隔离级别，事务中的每个一致读取都会设置并读取自己的新快照。使用“ LOCK IN SHARE MODE”时，将发生锁定读取：A SELECT阻塞，直到包含最新行的事务结束为止（请参见[第14.7.2.4节，“锁定读取””）（https://dev.mysql.com /doc/refman/5.7/en/innodb-locking-reads.html））。

一致读取不适用于某些DDL语句：

-一致的读取不能在`DROP TABLE`上进行，因为MySQL无法使用已删除的表并且`InnoDB`会破坏该表。
-一致的读取不能在“ ALTER TABLE”操作上起作用，该操作会创建原始表的临时副本并在构建临时副本时删除原始表。当您在事务内重新发出一致的读取时，新表中的行不可见，因为在获取事务快照时这些行不存在。在这种情况下，事务返回错误：`ER_TABLE_DEF_CHANGED`，“表定义已更改，请重试事务”。

对于没有指定FOR UPDATE或LOCK IN的select in子句，如INSERT INTO ... SELECT，UPDATE ...（SELECT）和CREATE TABLE ... SELECT子句，读取类型有所不同。共享模式`：

-默认情况下，InnoDB在这些语句中使用更强的锁定，而SELECT部分​​的作用类似于READ COMMITTED，其中每次一致的读取（即使在同一事务中）也会设置并读取自己的新快照。
-要在这种情况下执行非锁定读取，请启用“ innodb_locks_unsafe_for_binlog”选项，并将事务的隔离级别设置为“ READ UNCOMMITTED”，“ READ COMMITTED”或“ REPEATABLE READ”，以避免对从选定内容读取的行设置锁定表。
