...
使用 `READ COMMITTED` 隔离级别时, 事务中的每次一致读取都会设置并读取自己的新快照。
使用 `LOCK IN SHARE MODE` 时, 将使用锁定读: `SELECT`可能会被阻塞, 直到包含最新行的事务结束为止。 请参考【Section 14.7.2.4, “Locking Reads”】。

一致读取不适用于某些DDL语句：

- 一致读取不能在 `DROP TABLE` 上进行, 因为MySQL无法使用已删除的表, 并且`InnoDB`会摧毁这个表。
- 一致读取不能在 `ALTER TABLE` 操作上起作用, 因为这个操作会创建原始表的临时副本,并在构建临时副本后删除原始表。 当我们在事务中重新发出一致读取时, 新表中的数据行是不可见的, 因为在获取事务快照时这些行不存在。 在这种情况下, 事务返回错误信息:`ER_TABLE_DEF_CHANGED`, “Table definition has changed, please retry transaction”.

对于没有指定 `FOR UPDATE` 或者 `LOCK IN SHARE MODE` 的各种 select, 其行为有所不同, 如 `INSERT INTO ... SELECT`, `UPDATE ... (SELECT)`, 以及 `CREATE TABLE ... SELECT`：

- 默认情况下, `InnoDB` 在这些语句中使用更强的锁, 而 `SELECT` 部分​​的行为类似于 `READ COMMITTED`, 即使在同一事务中, 其中的每次一致读取, 也会设置并读取自己的新快照。
