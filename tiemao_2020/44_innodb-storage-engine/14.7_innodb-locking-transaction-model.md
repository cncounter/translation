...
间隙锁是性能和并发度之间的一种权衡, 并且只在某些事务隔离级别中使用。
...
更多信息, 请参考: [Section 14.6.1.6, “AUTO_INCREMENT Handling in InnoDB”]()。
...
- 对于其他查询条件, `InnoDB` 会锁定扫描到的索引范围, 通过间隙锁或者临键锁来阻止其他会话在这个范围中插入值。 关于间隙锁和临键锁的信息, 请参见前面的内容: [14.7.1 InnoDB Locking](#14.7.1)。
...
由于禁用了间隙锁, 可能会产生幻读问题(phantom problem), 因为其他session可能会在间隙中插入新行。 有关幻像的信息, 请参见 [14.7.4, “Phantom Rows”](#14.7.4)。
...
#### 14.7.2.2 autocommit(自动提交), Commit(提交), Rollback(回滚)


在`InnoDB`中, 所有用户活动都在事务内部执行。 如果启用了自动提交模式(`autocommit`), 则每个SQL语句都会自己形成一个事务。
MySQL中每个新的会话连接, 默认情况下都是自动提交模式, 因此, 如果该SQL语句未返回错误, 则MySQL会在SQL语句执行之后自动提交。
...
如果某个会话通过 `SET autocommit = 0` 禁用了自动提交模式, 则该会话始终会有一个事务打开。 `COMMIT` 或者 `ROLLBACK` 语句会结束当前事务并开始一个新的事务。

如果会话禁用了“自动提交”, 在还没有明确提交最终事务的情况下, 断开连接或者会话结束, 则MySQL会执行事务回滚。

有些语句会隐式地结束事务, 效果类似于在该语句之前自动增加了一个 `COMMIT` 语句。详细信息请参考【Section 13.3.3, “Statements That Cause an Implicit Commit”】。

`COMMIT` 表示将当前事务中所做的更改持久化(permanent), 并对其他会话可见。
而 `ROLLBACK` 语句则取消当前事务中的修改。
...
MySQL数据库的客户端连接, 默认情况下开启自动提交模式, 执行每个SQL语句时都会自动提交。
如果用过其他数据库管理系统, 可能对这种操作模式不太习惯。 我们更常用的方式, 是发出一连串的DML语句, 一起提交, 或者一起回滚。

想要使用多语句事务:
- 可以通过SQL语句 `SET autocommit = 0` 关闭自动提交, 并在适当的时机时以 `COMMIT` 或者 `ROLLBACK` 结束事务。
- 如果处于自动提交状态, 可以通过 `START TRANSACTION` 开始每个事务, 并以 `COMMIT` 或者 `ROLLBACK` 结束。

以下示例演示了两个事务。 其中, 第一个事务被提交； 第二个事务被回滚。
...
在 PHP, Perl DBI, JDBC, ODBC, 或者是标准C调用接口之类的 MySQL客户端API中, 可以将事务控制语句(如`COMMIT`)当做字符串发送给MySQL服务器, 就像普通的SQL语句一样(如`SELECT` 或 `INSERT`)。 某些API还单独提供了提交事务和回滚的函数/方法。
...
要实现一致性读(consistent read), 意味着 `InnoDB` 需要通过多版本技术, 为一个查询呈现某个时间点上的数据库快照。
查询能看到这个时间点之前所有已提交事务的更改, 而看不到这个时间点之后新开的事务、或者未提交的事务的更改。
例外的规则是可以看到同一事务中前面的语句所做的更改。 这种例外会导致一些异常:  如果更新table中的某些行, 则 `SELECT` 将看到更新后的行的最新版本, 但看到其他的数据行可能是旧版本。 如果其他会话也在更新同一张表, 则异常意味着有可能会看到某种并不存在的状态。

如果事务隔离级别是默认的 `REPEATABLE READ`, 则同一事务中的所有一致读, 都会读取该事务中第一次读取所建立的快照。 我们可以提交当前事务, 并在此之后发出新查询来获取最新的快照。

使用 `READ COMMITTED` 隔离级别时, 事务中的每次一致性读, 都会设置并读取自己的新快照。

一致性读是`InnoDB` 处理 `SELECT` 语句的默认模式, 支持 `READ COMMITTED` 和 `REPEATABLE READ` 隔离级别。
一致性读不会在读取的表上设置任何锁, 所以在读取时, 其他会话可以自由对这些表执行修改。

假设使用默认的 `REPEATABLE READ` 隔离级别。 在我们执行普通的 `SELECT` 一致性读时, `InnoDB` 会为当前事务指定一个时间点, 事务中的所有查询都根据这个时间点来确定可以看到哪些数据。 如果另一个事务在给定时间点后删除了一行数据并提交, 那么我们的事务则看不到该行已被删除。插入和更新的处理方式也是一样的。
...
使用 `LOCK IN SHARE MODE` 时, 将使用锁定读: `SELECT`可能会被阻塞, 直到包含最新行的事务结束为止。 请参考【Section 14.7.2.4, “Locking Reads”】。

一致性读不适用于某些DDL语句：

- 一致性读不能在 `DROP TABLE` 上进行, 因为MySQL无法使用已删除的表, 并且`InnoDB`会摧毁这个表。
- 一致性读不能在 `ALTER TABLE` 操作上起作用, 因为这个操作会创建原始表的临时副本,并在构建临时副本后删除原始表。 当我们在事务中重新发出一致性读时, 新表中的数据行是不可见的, 因为在获取事务快照时这些行不存在。 在这种情况下, 事务返回错误信息:`ER_TABLE_DEF_CHANGED`, “Table definition has changed, please retry transaction”.

对于没有指定 `FOR UPDATE` 或者 `LOCK IN SHARE MODE` 的各种 select, 其行为有所不同, 如 `INSERT INTO ... SELECT`, `UPDATE ... (SELECT)`, 以及 `CREATE TABLE ... SELECT`：

- 默认情况下, `InnoDB` 在这些语句中使用更强的锁, 而 `SELECT` 部分​​的行为类似于 `READ COMMITTED`, 即使在同一事务中, 其中的每次一致性读, 也会设置并读取自己的新快照。
- 要在这种情况下执行非锁定读取, 请启用 `innodb_locks_unsafe_for_binlog` 选项, 并将事务隔离级别设置为`READ UNCOMMITTED`, `READ COMMITTED`, 或者 `REPEATABLE READ`, 以避免从表中读取数据行时设置锁。
...
#### 14.7.2.4 锁定读(Locking Read)

如果在一个事务中, 先查询数据, 然后再insert或update相关数据, 则常规的`SELECT`语句并不能提供足够的保护。
其他事务可以更新或删除刚查询到的这些行。
`InnoDB` 支持两种类型的锁定读, 提供了额外的安全性:

- `SELECT ... LOCK IN SHARE MODE`

  在读取到的所有行上设置共享锁。 其他会话可以读取行, 但是当前事务提交之前都不能修改。 如果查询时, 某些行被另一个事务更改但尚未提交, 则会阻塞当前查询, 直到那些事务结束, 然后使用最新值。

- `SELECT ... FOR UPDATE`

  对于搜索到的索引记录, 锁定数据行以及所有关联的索引条目, 就如同对这些行执行了 `UPDATE` 语句一样。 阻塞其他事务更新这些行, 阻塞`SELECT ... LOCK IN SHARE MODE`, 甚至会阻塞以某些事务隔离级别的读取操作。 一致性读则会忽略读取视图中那些记录上设置的任何锁。 (记录的旧版本无法被锁定, 他们一般是通过记录的内存副本上加上 undo logs 来重建的)。

这类子句在处理树结构(tree-structured)或图结构(graph-structured)的数据时非常有用, 不管是使用单个表还是多个表来存储。 可以进行遍历, 同时还可以返回去修改其中任何一条记录。

当事务被提交或者回滚时, 由 `LOCK IN SHARE MODE` 和 `FOR UPDATE` 查询设置的所有锁都会被释放。
...
> ##### 注意
> 只有在禁用 autocommit 时, 才可以进行锁定读。（一般使用 `START TRANSACTION` 语句或者设置 `autocommit=0`）

在外部查询中的锁定读语句, 不会对子查询的数据行加锁, 除非子查询也指定了锁定读。
例如, 下面的语句不会锁定 `t2` 表中的行。
...
> 类比: parent -- 主表: master； child  -- 子表: detail

示例1:

假设需要在 `child` 表中插入新行, 但要确保在 `parent` 表中具有对应的父记录。 那么应用程序代码中, 可以通过以下顺序的操作, 来确保引用完整性。

首先, 使用一致性读来查询 `PARENT` 表并检查父记录是否存在。 那我们可以安全地将子行插入`CHILD`表吗？ 答案是否定的, 因为其他会话可能会在我们不知道的情况下, 在 `SELECT` 和 `INSERT` 之间, 恰好把 `PARENT `表中的那行数据删除了。

要避免这个潜在的问题,  加上 `LOCK IN SHARE MODE` 来执行`SELECT`:
...
在 `LOCK IN SHARE MODE` 查询返回父记录 `'Jones'` 之后,  我们可以安全地将子记录添加到`CHILD`表中, 并提交事务。 其他事务如果试图获取 `PARENT` 表中对应数据行的排他锁, 则会阻塞并等待我们完成操作之后才能继续, 也就是需要等这两张表中的数据处于一致状态。
...
又比如, `CHILD_CODES` 表中有一个整型的计数器字段, 用来为 `CHILD` 表中的每条记录分配唯一ID。
不能使用一致性读或者共享模式来读取计数器的当前值, 因为这样可能会有多个客户端看到相同的值,  如果两个事务尝试使用相同的id来添加数据, 则会发生Key重复错误(duplicate-key error)。

在这个场景下, `LOCK IN SHARE MODE` 不是一种好的解决方案,  因为如果有两个用户同时读取计数器, 则其中至少有一个会在尝试更新计数器时陷入死锁状态。

要实现计数器读取和递增, 需要先用 `FOR UPDATE` 执行对计数器的锁定读, 然后再递增计数器。例如:
...
`SELECT ... FOR UPDATE` 会读取最新的可用数据, 并在读到的每一行上设置排他锁。
因此, 它设置的锁, 与 `UPDATE` 语句在行中设置的锁相同。

这个示例仅仅是为了演示 `SELECT ... FOR UPDATE` 的工作机制。
在MySQL中, 生成唯一标识符的特定任务, 实际上可以通过单次查询来完成:
...
`SELECT` 语句只会查询基于当前连接的标识符信息。而且不读取任何表。
...
语句中是否有`WHERE`条件来过滤数据行都会这样。`InnoDB`并不会记下确切的WHERE条件, 只知道扫描到了哪些索引范围。 一般使用的是 临键锁(`next-key lock`), 这样就可以阻塞(block)新行插入到记录之前的间隙(gap)中。
...
如果在搜索中使用了二级索引, 并且要设置的索引记录锁是排他锁, 则`InnoDB`还会检索相应的聚簇索引记录(clustered index record)并对其上锁。
...
- `SELECT ... FROM` 是一致性读，读取数据库快照并且不设置锁定，除非将事务隔离级别设置为“ SERIALIZABLE”。对于“ SERIALIZABLE”级别，搜索会在遇到的索引记录上设置共享的下一键锁定。但是，对于使用唯一索引来搜索唯一行的行来锁定的语句，仅需要索引记录锁定。

-对于“ SELECT ... FOR UPDATE”或“ SELECT ... LOCK IN SHARE MODE”，将为扫描的行获取锁，并预期将其释放给不符合包含在结果集中的行（例如，如果它们不符合“ WHERE”子句中给出的条件）。但是，在某些情况下，行可能不会立即被解锁，因为结果行与其原始源之间的关系在查询执行期间会丢失。例如，在一个“ UNION”中，表中的扫描（和锁定）行可能在评估它们是否符合结果集之前被插入到临时表中。在这种情况下，临时表中的行与原始表中的行之间的关系将丢失，并且直到查询执行结束后，后行才被解锁。

-`SELECT ... LOCK IN SHARE MODE`在搜索遇到的所有索引记录上设置共享的下一键锁定。但是，对于使用唯一索引来搜索唯一行的行来锁定的语句，仅需要索引记录锁定。
...
-“ SELECT ... FOR UPDATE”在搜索遇到的每条记录上设置独占的下一键锁定。但是，对于使用唯一索引来搜索唯一行的行来锁定的语句，仅需要索引记录锁定。

  对于索引记录，遇到搜索时，“ SELECT ... FOR UPDATE”会阻止其他会话执行“ SELECT ... LOCK IN SHARE MODE”或读取某些事务隔离级别。一致的读取将忽略读取视图中存在的记录上设置的任何锁定。

-`UPDATE ... WHERE ...`在搜索遇到的每条记录上设置排他的下一键锁定。但是，对于使用唯一索引来搜索唯一行的行来锁定的语句，仅需要索引记录锁定。

-当“ UPDATE”修改聚集索引记录时，将对受影响的辅助索引记录进行隐式锁定。在插入新的二级索引记录之前执行重复检查扫描时，以及在插入新的二级索引记录时，“ UPDATE”操作还会对受影响的二级索引记录进行共享锁定。

-`DELETE FROM ... WHERE ...`在搜索遇到的每条记录上设置专有的下一键锁定。 但是，对于使用唯一索引来搜索唯一行的行锁定的语句，仅需要索引记录锁定。
...
-`INSERT`在插入的行上设置排他锁。该锁是索引记录锁，不是下一个键锁（即没有间隙锁），并且不会阻止其他会话插入到插入行之前的间隙中。
  ...
  在插入行之前，设置了一种称为插入意图间隙锁的间隙锁。此锁发出插入意图的信号是，如果多个事务未插入间隙中的相同位置，则无需等待彼此插入的多个事务。假设有索引记录，其值分别为4和7。尝试在插入行上获得排他锁之前，分别尝试插入5和6的值的每个事务都使用插入意图锁来锁定4和7之间的间隙，但是没有彼此阻塞，因为行没有冲突。

  如果出现重复键错误，则会在重复索引记录上设置一个共享锁。如果另一个会话已经具有互斥锁，则如果有多个会话试图插入同一行，则使用共享锁可能会导致死锁。如果另一个会话删除该行，则会发生这种情况。假设InnoDB表t1具有以下结构：
...
-INSERT ... ON DUPLICATE KEY UPDATE与简单的INSERT不同，在发生重复键错误时，将排他锁而不是共享锁放在要更新的行上。 对重复的主键值采用排它索引记录锁定。 唯一的下一键锁定用于重复的唯一键值。

-如果唯一键上没有冲突，则完成`REPLACE`就像`INSERT`。 否则，排他的下一键锁将放置在要替换的行上。
...
-`INSERT INTO T SELECT ... FROM S WHERE ...`在插入`T`的每一行上设置排他索引记录锁（不带间隙锁）。如果事务隔离级别为“ READ COMMITTED”，或者启用了“ innodb_locks_unsafe_for_binlog”，并且事务隔离级别为“ SERIALIZABLE”，则InnoDB会对S进行搜索，以使其保持一致的读取（无锁）。否则，`InnoDB`在`S`的行上设置共享的下一键锁。在后一种情况下，InnoDB必须设置锁：在使用基于语句的二进制日志进行前滚恢复期间，必须以与原始操作完全相同的方式执行每个SQL语句。

  CREATE TABLE ... SELECT ...`使用共享的下一键锁或作为一致的读取来执行`SELECT`，如[`INSERT ... SELECT`]（https://dev.mysql.com/ doc / refman / 5.7 / en / insert-select.html）。

  当在构造中使用“ SELECT”时，将“ REPLACE INTO t SELECT ... FROM s WHERE ...”或“ UPDATE t ... WHERE col IN（SELECT ... FROM s ...）”中的InnoDB `设置表`s`的行上的共享的下一键锁。
...
-InnoDB在初始化表上先前指定的AUTO_INCREMENT列的同时，在与AUTO_INCREMENT列关联的索引的末尾设置排他锁。

   在`innodb_autoinc_lock_mode = 0`的情况下，`InnoDB`使用一种特殊的`AUTO-INC`表锁定模式，在这种模式下，获取该锁定并保持在当前SQL语句的末尾（而不是整个事务的末尾），同时访问自动模式。 -增量计数器。 持有“ AUTO-INC”表锁时，其他客户端无法插入该表。 对于带有[`innodb_autoinc_lock_mode = 1`]（）的“批量插入”，也会发生相同的行为。 表级“ AUTO-INC”锁不与[`innodb_autoinc_lock_mode = 2`]（）一起使用。 有关更多信息，请参见[第14.6.1.6节，“ InnoDB中的AUTO_INCREMENT处理”]（）。
...
-如果在表上定义了“ FOREIGN KEY”约束，那么任何需要检查约束条件的插入，更新或删除操作都会在它查看约束的记录上设置共享记录级锁。 在约束失败的情况下，InnoDB也会设置这些锁。

-`LOCK TABLES`设置表锁，但是设置这些锁的是`InnoDB`层之上的MySQL更高层。 如果innodb_table_locks = 1（默认值）和`autocommit = 0`，并且InnoDB之上的MySQL层知道行级锁，则InnoDB会意识到表锁。

   否则，InnoDB的自动死锁检测将无法检测到涉及此类表锁的死锁。 同样，由于在这种情况下，较高的MySQL层不知道行级锁，因此可以在另一个会话当前具有行级锁的表上获取表锁。 但是，这并不危及交易的完整性，如第14.7.5.2节“死锁检测”]（）所述。
...
-如果innodb_table_locks = 1（默认），则LOCK TABLES在每个表上获取两个锁。除了MySQL层上的表锁外，它还获得了一个InnoDB表锁。 4.1.2之前的MySQL版本没有获取InnoDB表锁；可以通过设置“ innodb_table_locks = 0”来选择旧的行为。如果没有获取“ InnoDB”表锁，那么即使表的某些记录被其他事务锁定，“ LOCK TABLES”也会完成。

  在MySQL 5.7中，`innodb_table_locks = 0`对于用LOCK TABLES ... WRITE显式锁定的表无效。对于通过“锁定表...写”隐式地（例如，通过触发器）或通过““锁定表...读”]（）锁定的表进行读取或写入，确实有作用。

-提交或中止事务时，将释放事务持有的所有“ InnoDB”锁。因此，以[`autocommit = 1`]（）模式在`InnoDB`表上调用`LOCK TABLES'没有多大意义，因为获取的`InnoDB`表锁将立即释放。

-您不能在事务中间锁定其他表，因为`LOCK TABLES`执行隐式的`COMMIT'和[`UNLOCK TABLES`]（）。
...
在一个事务中, 相同的查询语句, 在不同的时间点执行时, 如果产生了不同的结果集, 这种现象就称为幻读(phantom problem)。
示例: 同一个 `SELECT`语句执行两次, 但第二次返回的结果多了1行, 在第一次查询结果中是没有的, 那么这一行就是"幻影行(Phantom Rows)"。

假设 `child` 表的 `id` 列有索引, 我们想要获取所有id值大于`100`的行并进行锁定, 以便稍后进行更新:
...
如果表中有两行的id值为`90`和`102`。
如果在扫描范围内对索引记录设置的锁定未锁定在间隙中插入（在本例中为 90到102之间的间隙）, 另一个会话可以在表中插入一个id为101的新行。如果要在同一事务中执行相同的SELECT,  , 您将会在查询返回的结果集中看到一个新行, 其id为101（"幻影"​​）。如果我们将一组行视为数据项, 则新的幻影子级将违反事务应能够运行的事务隔离原则, 以使已读取的数据在事务期间不会更改。

为了防止产生幻象, InnoDB使用一种称为"下一键锁定"的算法, 该算法结合了索引行锁定和间隙锁定。 InnoDB执行行级锁定的方式是, 当它搜索或扫描表索引时, 会在遇到的索引记录上设置共享或互斥锁。因此, 行级锁实际上是索引记录锁。此外, 索引记录上的下一键锁定也会影响该索引记录之前的"间隙"。即, 下一键锁定是索引记录锁定加上索引记录之前的间隙上的间隙锁定。如果一个会话在索引中的记录" R"上具有共享或排他锁, 则另一会话不能在索引顺序中紧接在" R"之前的间隙中插入新的索引记录。
...
当InnoDB扫描索引时，它也可以锁定索引中最后一条记录之后的间隔。只是在前面的示例中发生了这种情况：为了防止在表中插入“ id”大于100的任何内容，InnoDB设置的锁包括对“ id”值102之后的间隙的锁定。
...
如第14.7.1节“ InnoDB锁定”]（）所述，可以禁用间隙锁定。这可能会导致幻影问题，因为在禁用间隙锁定时，其他会话可以在间隙中插入新行。
