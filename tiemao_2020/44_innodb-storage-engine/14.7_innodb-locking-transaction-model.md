...
当同一查询在不同时间产生不同的行集时，在事务内就会发生所谓的幻像问题。 例如，如果一次执行SELECT两次，但第二次返回的行却不是第一次返回，则该行是“幻像”行。

假设在“子”表的“ id”列上有一个索引，并且您想要读取并锁定表中所有标识符值大于100的行，以更新所选行中的某些列 后来：
...
该查询从“ id”大于100的第一条记录开始扫描索引。让表包含“ id”值分别为90和102的行。如果在扫描范围内对索引记录设置的锁定未锁定在间隙中插入（在本例中为90与102之间的间隙），另一个会话可以在表中插入一个id为101的新行。如果要在同一事务中执行相同的SELECT， ，您将会在查询返回的结果集中看到一个新行，其id为101（“幻影”​​）。如果我们将一组行视为数据项，则新的幻影子级将违反事务应能够运行的事务隔离原则，以使已读取的数据在事务期间不会更改。
