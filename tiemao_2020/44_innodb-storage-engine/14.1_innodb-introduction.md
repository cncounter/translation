...
InnoDB是一种通用的存储引擎，可兼顾高可靠性和高性能。 在MySQL 5.7中，InnoDB是默认的MySQL存储引擎。 除非您配置了其他默认存储引擎，否则发出不带ENGINE =子句的CREATE TABLE语句将创建InnoDB表。
...
- 其“ DML”操作遵循“ ACID”模型，具有“交易”功能，具有“提交”，“回滚”和[崩溃恢复]功能来保护用户数据。有关更多信息，请参见[第14.2节“ InnoDB和ACID模型”]（https://dev.mysql.com/doc/refman/5.7/en/mysql-acid.html）。
- 行级“锁定”和Oracle风格的[consistent reads]提高了多用户并发性和性能。有关更多信息，请参见[第14.7节，“ InnoDB锁定和事务模型”]（https://dev.mysql.com/doc/refman/5.7/en/innodb-locking-transaction-model.html）。
- `InnoDB`表将数据安排在磁盘上，以基于[主键]优化查询。每个`InnoDB`表都有一个称为[clustered index]的主键索引，该索引组织数据以最小化主键查找的I / O。有关更多信息，请参见[第14.6.2.1节，“聚集索引和二级索引”]（https://dev.mysql.com/doc/refman/5.7/en/innodb-index-types.html）。
- 为了维护数据完整性，InnoDB支持[FOREIGN KEY]约束。使用外键检查插入，更新和删除操作，以确保它们不会导致不同表之间的不一致。有关更多信息，请参见[第13.1.18.5节，“外键约束”]（https://dev.mysql.com/doc/refman/5.7/en/create-table-foreign-keys.html）。
...
- 有关与InnoDB相关的术语和定义，请参见[MySQL词汇表]（https://dev.mysql.com/doc/refman/5.7/zh/glossary.html）。
- 有关InnoDB存储引擎的论坛，请参见[MySQL Forums :: InnoDB]（http://forums.mysql.com/list.php?22）。
- InnoDB与MySQL在相同的GNU GPL许可证版本2（1991年6月）下发布。 有关MySQL许可的更多信息，请参见http://www.mysql.com/company/legal/licensing/。
...
- InnoDB存储引擎维护自己的[缓冲池]，在访问数据时将表和索引数据缓存在主内存中。经常使用的数据直接从内存中处理。此缓存适用于多种类型的信息，并加快了处理速度。在专用数据库服务器上，通常最多将80％的物理内存分配给缓冲池。
...
- 插入，更新和删除通过称为[更改缓冲]`的自动机制进行了优化。 InnoDB不仅允许对同一表的并发读写访问，而且还缓存更改的数据以简化磁盘I / O。
...
- 即使在同一条语句中，您也可以将InnoDB表与其他MySQL存储引擎的表自由混合。 例如，您可以使用“ join”操作来合并InnoDB和[`MEMORY`]中的数据（https://dev.mysql.com/doc/refman/5.7/en/memory-storage-engine.html ）表中的单个查询。
- InnoDB是为处理大量数据时的CPU效率和最佳性能而设计的。
...
有关可以在应用程序代码中应用的特定于InnoDB的调优技术，请参见[第8.5节，“优化InnoDB表”]（https://dev.mysql.com/doc/refman/5.7/zh/optimizing-innodb .html）。
...
- 不使用[`LOCK TABLES`]（https://dev.mysql.com/doc/refman/5.7/en/lock-tables.html）语句。 InnoDB可以一次处理多个会话，一次读写同一张表，而不会牺牲可靠性或高性能。要获得对一组行的排他写访问权限，请使用[`SELECT ... FOR UPDATE`]（https://dev.mysql.com/doc/refman/5.7/en/innodb-locking-reads.html）语法仅锁定要更新的行。
...
如果InnoDB不是您的默认存储引擎，则可以通过使用命令行上定义的--default-storage-engine = InnoDB重新启动服务器，来确定数据库服务器或应用程序是否可以与InnoDB一起正常工作。在MySQL服务器选项文件的``mysqld''部分中定义的default-storage-engine = innodb`。
...
如果您没有对存储引擎做出明智的决定，并且想预览使用InnoDB创建时某些表的工作方式，请发出命令[ALTER TABLE table_name ENGINE = InnoDB;]（https：// dev。 mysql.com/doc/refman/5.7/en/alter-table.html）。或者，要在不干扰原始表的情况下运行测试查询和其他语句，请进行复制：
...
Oracle建议将InnoDB作为典型数据库应用程序的首选存储引擎，从在本地系统上运行的单用户Wiki和博客，到提高性能极限的高端应用程序。 在MySQL 5.7中，InnoDB是新表的默认存储引擎。

>重要

InnoDB无法禁用。 “ --skip-innodb”选项已被弃用，并且无效，并且使用它会产生警告。 希望在将来的MySQL版本中将其删除。 这也适用于其同义词（“ --innodb = OFF”，“-disable-innodb”等）。
