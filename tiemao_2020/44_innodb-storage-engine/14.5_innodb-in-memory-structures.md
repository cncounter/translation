...
![更改缓冲区(Change Buffer)](https://dev.mysql.com/doc/refman/5.7/en/images/innodb-change-buffer.png)
...
当受影响的行太多, 需要更新的二级索引数据量很大时, 更改缓冲区的合并可能要消耗很多个小时。
在此期间, 磁盘 I/O 会增加, 这可能会导致有磁盘读取的查询操作的执行速度明显变慢。
事务提交之后, 甚至在服务器关闭并重新启动之后, 更改缓冲区的合并都有可能持续发生( 更多信息请参考 [Section 14.22.2, “Forcing InnoDB Recovery”](https://dev.mysql.com/doc/refman/5.7/en/forcing-innodb-recovery.html) )。
...
在内存中, 更改缓冲区占用了缓冲池的一部分内存空间。
在磁盘上, 更改缓冲区是系统表空间的一部分，当数据库服务器关闭时，索引的变更会存储在里面。

更改缓冲区中缓存的数据类型由 [`innodb_change_buffering`](https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_change_buffering) 变量控制。 更多信息请参见 [Configuring Change Buffering](https://dev.mysql.com/doc/refman/5.7/en/innodb-change-buffer.html#innodb-change-buffer-configuration)。
我们还可以配置更改缓冲区的最大空间。 更多信息请参见 [Configuring the Change Buffer Maximum Size](https://dev.mysql.com/doc/refman/5.7/en/innodb-change-buffer.html#innodb-change-buffer-maximum-size)。

如果主键索引或者二级索引包含降序的索引列，则二级索引不支持更改缓冲操作。

有关更改缓冲区的常见问题和答案，请参见 [Section A.16, “MySQL 5.7 FAQ: InnoDB Change Buffer”](https://dev.mysql.com/doc/refman/5.7/en/faqs-innodb-change-buffer.html)。
