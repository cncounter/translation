...
我们还可以配置写缓冲的最大空间。 更多信息请参见 [Configuring the Change Buffer Maximum Size](#innodb-change-buffer-maximum-size)。
...
[`innodb_change_buffer_max_size`]（https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_change_buffer_max_size）变量允许将更改缓冲区的最大大小配置为总大小的百分比缓冲池。默认情况下，[`innodb_change_buffer_max_size`]（https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_change_buffer_max_size）设置为25。最大设置为50。

考虑在具有大量插入，更新和删除活动的MySQL服务器上增加[`innodb_change_buffer_max_size`]（https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_change_buffer_max_size）缓冲区合并不能跟上新的更改缓冲区条目，从而导致更改缓冲区达到其最大大小限制。

考虑在具有静态数据的MySQL服务器上减少[`innodb_change_buffer_max_size`]（https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_change_buffer_max_size），或者是否使用更改缓冲区消耗太多与缓冲池共享的内存空间，导致页面比期望的更快地过期。

使用代表性的工作负载测试不同的设置，以确定最佳配置。 [`innodb_change_buffer_max_size`]（https://dev.mysql.com/doc/refman/5.7/en/innodb-parameters.html#sysvar_innodb_change_buffer_max_size）的设置是动态的，它允许在不重新启动服务器的情况下修改设置。
