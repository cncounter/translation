...
- 服务器在启动时将成本模型表读入内存，并在运行时使用内存中的值。表中指定的任何非“NULL”成本估计优先于相应的编译默认成本常量。任何“NULL”估计都向优化器表明使用编译的默认值。
- 在运行时，服务器可能会重新读取成本表。这发生在动态加载存储引擎或执行 [`FLUSH OPTIMIZER_COSTS`](https://dev.mysql.com/doc/refman/8.0/en/flush.html#flush-optimizer-costs) 语句时.
- 成本表使服务器管理员能够通过更改表中的条目轻松调整成本估算。通过将条目的成本设置为“NULL”也很容易恢复为默认值。优化器使用内存中的成本值，因此对表的更改应遵循 [`FLUSH OPTIMIZER_COSTS`](https://dev.mysql.com/doc/refman/8.0/en/flush.html#flush-优化器成本）生效。
- 客户端会话开始时的当前内存中成本估计适用于整个会话，直到它结束。特别是，如果服务器重新读取成本表，则任何更改的估计仅适用于随后启动的会话。现有会话不受影响。
