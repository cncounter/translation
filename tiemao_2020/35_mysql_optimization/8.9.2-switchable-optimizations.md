...
- 批量Key访问标志(Batched Key Access Flag)
...
- 块嵌套循环标志(Block Nested-Loop Flag)
  ...
  * [`block_nested_loop`](https://dev.mysql.com/doc/refman/8.0/en/switchable-optimizations.html#optflag_block-nested-loop) (默认值为 `on`)
   ...
   控制 BNL 连接算法的使用。 在 MySQL 8.0.18 及更高版本中，这也控制哈希连接(hash join)的使用，以及 [`BNL`](https://dev.mysql.com/doc/refman/8.0/en/optimizer-hints.html#optimizer-hints-table-level) 和 `NO_BNL` 优化器提示。 在 MySQL 8.0.20 及更高版本中， MySQL 服务器去掉了对块嵌套循环支持，这个标志仅用于控制 hash join 的使用。
...
- 派生表合并标志
  ...
  * [`derived_merge`](https://dev.mysql.com/doc/refman/8.0/en/switchable-optimizations.html#optflag_derived-merge) (default `on`); 控制派生表和视图是否合并到外部查询块中。
