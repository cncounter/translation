...
commands中的命令顺序无关紧要，如果存在 `default` 命令，则会最先执行。 将 `opt_name` 标志设置为 `default` 会将其设置为 `on` 或 `off` 中的任何一个为其默认值。 不允许在值中多次指定任何给定的 `opt_name`，这会导致错误。 值中的任何错误都会导致赋值失败并出现错误，留下值 [`optimizer_switch`](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html# sysvar_optimizer_switch) 不变。
