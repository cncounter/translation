...
[`optimizer_switch`](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html#sysvar_optimizer_switch) 系统变量可以控制优化器的行为。 它的值是一组标志，每个标志都有一个“on”或“off”的值来指示相应的优化器行为是启用还是禁用。 此变量具有全局和会话值，可以在运行时更改。 可以在服务器启动时设置全局默认值。
