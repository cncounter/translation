...
如果你看过往期的问题，就会发现每一个都不简单。 都是模拟了认证考试中的难题。 而 “中级(intermediate)” 和 “高级(advanced)” 则是指的试题难度，而不是说这些知识本身很难。 嗯，一般来说，“高级”问题会稍微难一点。
...
开发中我们经常使用包装类（例如 `Boolean`, `Double`, 以及 `Integer` 等等）。 请看下面的代码片段：
...
这个问题考察原生数据的包装类，主要是 `Boolean` 类比较偏僻的 `valueOf` 工厂方法。
在实际考试中，这个问题可能不太容易碰到，因为它主要还是靠死记硬背， 大部分考试都会避免此类问题。
但是，这个问题从多个方面综合考察了对Java语言的理解和认识， 有点小坑，关键在于解答的过程。
...
- 2. 如果语义很清晰， 在代码中将原生数据类型赋值给包装类的变量，则会发生自动装箱 (autoboxing)。 自动装箱只是语法上的简写，它允许编译器(`javac`)自动生成`valueOf`方法调用, 主要目的是为了书写更简洁的代码。
- 3. 第三种方法是使用构造器, 也就是通过 `new` 关键字来调用构造函数。 实际上，在 Java 9 中已经不推荐使用第三种方法，而本文的一个目标就是解释为什么不赞成使用它。
...
在Java中，只要使用 `new` 关键字来调用构造函数，只会发生两种情况： 要么创建并返回指定类型的新实例，要么就是抛出异常。
这实际上是一个限制，现在一般是首选工厂方法， 因为除了达成这两种效果之外，工厂方法可能还会有一些优化。
...
工厂方法的一种功能是构造函数实现不了的:  就是返回与请求参数相匹配的，已存在的实例对象。
因为 `Integer` 包装器是不可变的， 表示相同数值的两个`Integer`对象也就完全是可以互换的。
因此，创建两个表示相同值的对象会浪费内存。
此外，这种方法允许使用 `==` 来比较两个对象， 而不必每次都写成 `equals(Object o)` 方式。
对于 `Integer` 类，一般来说是缓存了 `-128 到 +127` 范围内的值。
...
> 这种行为实际上类似于在编码中直接使用 `"XXX"` 这种字面量表示方式， 而不是 `new String("XXX")`。
...
工厂方法还有两个优点。
如果创建了多个工厂，则每个方法可以使用不同的名称，名称不同也就可以使用相同的参数声明。
对于构造函数而言，这样做是不可能的，因为必须参数类型不同才能形成重载。
...
第三个优点是, Java中的 `new ` 调用构造函数只能返回类型完全相同的对象。
而工厂可以返回与声明类型兼容的各种类型（例如接口的实现类，而且这是隐藏实现细节的绝佳方法）。
...
这个问题最关键的地方在于， 我们使用 `Boolean.valueOf(...)` 时，只会得到两个常量对象： `Boolean.TRUE` 和 `Boolean.FALSE`。
这两个对象可以被重复利用，而不会浪费多余的内存。 执行 `new` 调用显然是不可能的。
...
大部分包装类的工厂方法, 如果传入了 `null` 参数, 或者字符串参数不符合目标值的表现形式（例如，`Integer.valueOf("six")`) 就会抛出异常。

但 `java.lang.Boolean` 类的工厂方法是个特例， 判断的是非空(`null`)并且等于 "`true`"（字符串忽略大小写）。
如果满足这个条件则返回 `Boolean.TRUE`。 否则直接返回 `Boolean.FALSE`。
这意味着如果传入 `null` 或者无意义的字符串, 则会返回 `Boolean.FALSE`，不会抛出异常。
...
一般来说 `if` 语句的测试表达式必须是 `boolean` 类型。
很显然，这里将 `Boolean` 对象进行拆箱操作, 变为 `boolean` 类型。
这算是Java基础知识，当然，如果在 Java 5 之前的版本这样写, 代码确实无法编译。
当然，即使有这样的担忧，但因为没有【编译错误】这个选项，所以我们不用关注这个问题。
...
在这种情况下，我们已经确定 `b1` 所引用的对象相当于 `false` 值。 因此，`if`判断不通过，里面的代码也就未执行。
由此，我们可以确定 `选项D是正确的`。
...
尽管我们确定 `if` 语句内部的代码没有执行，但是面试过程中可能会问： 如果执行了的话是什么结果。
...
每个对象都可以使用 `equals(Object o)` 方法，因为是在 `java.lang.Object` 类中定义的。
当然，除非某个类覆写了equals方法，否则不一定会有用。
我们这里主要是讨论 `==` 运算符，如果对 equals 的实现感兴趣, 请参考: [Java中hashCode与equals方法的约定及重写原则](https://blog.csdn.net/renfufei/article/details/14163329)。
...
`==` 运算符比较两个表达式的值。
听起来很简单，但是表达式的值可能是两种不同的类型。
因为两种类型之间 `==` 的结果可能会不同。
顺便说一下，这里故意使用术语“`表达式`”， 变量也是一个简单的表达式。
...
- 原生数据类型( `primitive`, 共8种: `boolean`, `byte`, `short`, `char`, `int`, `long`, `float`, `double`)
- 引用类型(`reference`)。  引用非常类似于指针， 表示在内存中某个对象的地址值(可以认为是一个偏移量数值)。
...
如果表达式是原生数据类型，则表达式的值很直观。
例如，如果 `int` 表达式的值为`32`，则该表达式的值就是`32`的二进制表示形式。

但问题是，如果变量是引用类型（例如，`Integer` 类型）， 它所引用对象内部的值为`32`，那么这个变量的值并`不是32`。
这是一个神秘的数字（一个引用地址），通过这个引用地址，JVM可以找到对应的 `Integer` 对象。
也就是说，对于引用类型，也就是除了那8种原生数据类型之外的所有类型， `==` 表达式判断的都是引用的内存地址是否相等，也就是判断它们是否引用了同一个对象。
最重要的是，如果两个 `Integer` 对象里面的值都是 `32`，但它们是不同的对象， 那么它们的引用地址也就不同，然后使用`==`比较则会返回 `false`。
...
输出肯定是 `false`。 前面也提到过，`new` 的任何调用，要么产生一个全新的对象, 要么抛出异常。
这意味着 `v2` 和 `v1` 引用了不同的对象，`==` 操作的结果是 `false`。
...
这与最初的代码非常相似，一个使用构造函数, 一个使用自动装箱，可以肯定这也会输出 `false`。
构造函数创建的对象必定是是唯一的新对象，因此，不可能是自动装箱的工厂方法返回的对象。
...
不可变对象的工厂方法一般会特殊处理，只要参数相等，就返回同一个（缓存的）对象。

`Integer` 类的API文档中，对 `valueOf(int)` 方法有如下说明：

“This method will always cache values in the range -128 to 127, inclusive, and may cache other values outside of this range.”

“此方法将始终缓存 `[-128 ~ 127]` 范围内的值， 还可能会缓存这个范围之外的其他值。”
...
虽然文档中只在 `valueOf(int)` 和 `valueOf(String)` 方法的说明中提到了这个缓存保证。 但在实际的实现中， 其他包装类也表现出相同的合并行为。
...
当然，这里讨论了两个 `Integer` 对象：一个是使用构造函数创建的，另一个是使用自动装箱创建的（使用 `Integer.valueOf(int)` 方法）。
假如我们稍微改变一下面试题中 `if` 语句的判断，则输出内容将为 false。

总结: 本文开始提到的面试题， 选项D是正确答案。 这里的讨论只是个附带。
