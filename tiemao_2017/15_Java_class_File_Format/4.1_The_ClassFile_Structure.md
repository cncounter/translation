...
中的每个值`methods`表必须`method_info`结构((§4.6)(# jvm - 4.6))给的完整描述一个方法在这类或接口。如果既不`ACC_NATIVE`和`ACC_ABSTRACT`flags are in the设置`access_flags`项目的`method_info`结构、实现方法的Java虚拟机指令也supplied.The`method_info`结构代表所有声明的方法的类或接口类型,包括实例方法,类方法,实例初始化方法(§2.9(https://docs.oracle.com/javase/specs/jvms/se8/html/jvms html # jvm - - 2. - 2.9)),以及任何类或接口初始化方法((§2.9)(https://docs.oracle.com/javase/specs/jvms/se8/html/jvms html # jvm - - 2. - 2.9))。的`methods`表不包括项目代表从超类继承的方法或它的超接口。
...
的值`attributes_count`物品属性的数量`attributes`这个类的表。
...
每个值的`attributes`表必须是一个`attribute_info`结构((§4.7)(# jvm - 4.7))。此规范定义的属性中出现的`attributes`表的`ClassFile`列出结构(表4.7 - c)(# jvm - 4.7 - 320)。有关属性出现在定义的规则`attributes`表的`ClassFile`结构给出了(§4.7)(# jvm - 4.7)。关于设定属性的规则`attributes`表的`ClassFile`结构给出了(§4.7.1)(# jvms-4.7.1)。
