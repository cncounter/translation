...
本章主要介绍 Java虚拟机的 `class` 文件格式。每个 `class` 文件包含一个类或一个接口(class or interface)的定义信息. 有的类或接口并不一定要有外部文件来表述其字面信息(例如, 通过 class loader 生成的类), 但一般将类或接口的有效表示方式称为 `class` 文件格式。
...
`class` 文件由 8-bit 的字节流组成。所有的16位、32位和64位的量都是由连续的 2个,4个,8个字节组成. 多字节的数据项总是存储为大端字节序(big-endian order), 高位字节放在前面。在Java SE平台上, 所有实现 `java.io.DataInput` 和 `java.io.DataOutput` 接口的类都支持这种格式， 如 `java.io.DataInputStream` 以及 `java.io.DataOutputStream` 类。
...
- minor_version, major_version
  ...
  可能会下令字母顺序进行的,例如,1.5 < 2.0 < 2.1。可以支持Java虚拟机实现的类文件格式版本v当且仅当谎言在一些连续的范围Mi.0 v≤≤Mj.m。Java SE平台的版本的Java虚拟机实现符合负责确定范围。甲骨文Java虚拟机实现在JDK版本1.0.2支持类的文件格式版本45.0到45.0包容性。JDK 1.1版本。*支持类的文件格式版本在45.0到45.65535范围包容性。对于k≥2,JDK版本1。k支持类的文件格式通过44 +版本在45.0 k。0包容性。
