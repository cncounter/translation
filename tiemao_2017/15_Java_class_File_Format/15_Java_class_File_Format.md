...
- access_flags
  ...
  `access_flags`,访问控制标识, 由多个标志位组成, 用来表示 class 或 interface 机器属性的访问权限。每个标志位的含义如下表所示, [Table 4.1-A](#jvms-4.1A)。

  **Table 4.1-A.  class 访问控制与 property 修饰符**

  | Flag Name        | Value  | Interpretation                           |
  | ---------------- | ------ | ---------------------------------------- |
  | `ACC_PUBLIC`     | 0x0001 | 声明为 `public`; 可以被其他包(package)中的类访问到(accessed, 看见) . |
