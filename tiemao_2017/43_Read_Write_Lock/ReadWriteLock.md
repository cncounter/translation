...
一个`ReadWriteLock`保持一双相关(`locks`)(https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/locks/Lock.html),一个用于只读操作,一个用于写作。(`read lock`)(https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/locks/ReadWriteLock.html)readLock——可能是由多个读线程同时举行,只要没有作家。(`write lock`](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/locks/ReadWriteLock.html writeLock——)是排斥的。
