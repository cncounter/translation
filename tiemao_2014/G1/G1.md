...
1. 接下来讲述了 Hotspot JVM 中 CMS 收集器是如何工作的. 
...
并发标记清理(CMS, Concurrent Mark Sweep)收集器(也称为并发低暂停收集器)回收老年代内存(tenured generation). 它将垃圾回收中的绝大部分工作与应用程序的线程一起并发执行,以期能最小化暂停时间. 通常并发低暂停收集器不复制或也不压缩存活的对象. 垃圾回收不移动存活的对象, 如果产生内存碎片问题,就会分配/占用更大的堆内存空间。
...
<td>
在Java应用程序线程运行的同时遍历老年代(tenured generation)的可达对象图。扫描从被标记的对象开始,直到遍历完从root可达的所有对象. 调整器(mutators)在并发阶段的2、3、5阶段执行,在这些阶段中新分配的所有对象(包括晋升的对象)都立刻标记为存活状态.
...
![](03_1_CMS_Heap_Structure_CN.png)
...
![](03_3_Yong_Generation_Collection_CN.png)
...
图中新晋升的对象用深蓝色来标识. 绿色的部分是年轻代中存活的对象,但还没被提升到老年代中.
...
**备注:** 在撰写本文时,回收巨大无比的对象还没有被优化. 因此,您应该尽量避免创建这么大的对象.

#### 3. G1 中的年轻代(Young Generation) ####
...
![](04_3_Young_Generation_in_G1_CN.png)
...
![](04_5_End_of_Young_GC_with_G1_CN.png)
...
- Young generation garbage collections, or young GCs, are stop the world 事件. All application threads are stopped for the operation.
...
<td>
(Stop the World Event,JVM所有线程暂停事件)	This is a stop the world 事件. With G1, it is piggybacked on a normal young GC. Mark survivor regions (root regions) which may have references to objects in old generation.
</td>
</tr/>
...
<td>
(Stop the World Event,JVM所有线程暂停事件)	Completes the marking of live object in the heap. Uses an algorithm called snapshot-at-the-beginning (SATB) which is much faster than what was used in the CMS collector.
</td>
</tr/>
...
<td>
(Stop the World Event,JVM所有线程暂停事件 and Concurrent)	
...
<td>
(Stop the World Event,JVM所有线程暂停事件)	These are the stop the world pauses to evacuate or copy live objects to new unused regions. This can be done with young generation regions which are logged as [GC pause (young)]. Or both young and old generation regions which are logged as [GC Pause (mixed)].
</td>
</tr/>
...
##### 6. 初始化标记阶段(Initial Marking Phase) #####
...
![](04_7_Concurrent_Marking_Phase_CN.png)
...
![](04_10_After_Copying_Cleanup_Phase_CN.png)
