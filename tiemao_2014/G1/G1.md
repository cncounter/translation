...
年轻代(Young generation)分为 1个新生代空间(Eden)和2个存活区(survivor spaces). 老年代(Old generation)是一大块连续的空间, 垃圾收集(Object collection)就地解决(is done in place), 除了 Full GC, 否则不会进行压缩(compaction).
...
##### 3. 年轻代垃圾收集(Young Generation Collection) #####
...
##### 5. CMS的老年代收集(Old Generation Collection) #####
...
G1收集器采用一种不同的方式来分配堆. 下面通过图解的方式一步步地叙述G1系统.
