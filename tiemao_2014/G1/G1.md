...
老一代的垃圾收集器(串行serial, 并行parallel, 以及CMS)都把堆内存划分为固定大小的三个部分: 年轻代(young generation), 年老代(old generation), 以及持久代(permanent generation).

![](02_1_HeapStructure.png)


内存中的所有对象存放在这三个部分中的一个.

而 G1 收集器采用一种不同的方式来处理.
