...
There is a walkthrough of this application [later in this article](https://www.html5rocks.com/en/tutorials/webrtc/basics/#toc-simple).
...
3. Get to grips with the RTCPeerConnection API by reading through the [simple example below](https://www.html5rocks.com/en/tutorials/webrtc/basics/#simpleRTCPeerConnectionExample) and the demo at [simpl.info/pc](http://www.simpl.info/pc), which implements WebRTC on a single web page.
...
- [`MediaStream`](https://www.html5rocks.com/en/tutorials/webrtc/basics/#toc-mediastream) (aka `getUserMedia`)
- `RTCPeerConnection`
- [`RTCDataChannel`](https://www.html5rocks.com/en/tutorials/webrtc/basics/#toc-rtcdatachannel)
...
(There is detailed discussion of the network and signaling aspects of WebRTC [below](https://www.html5rocks.com/en/tutorials/webrtc/basics/#signaling).)
...
- A [constraints object](https://www.html5rocks.com/en/tutorials/webrtc/basics/#toc-constraints).
...
[![ASCII image generated by idevelop.ro/ascii-camera](https://www.html5rocks.com/en/tutorials/webrtc/basics/ascii.png)](http://idevelop.ro/ascii-camera/)gUM ASCII art!
...
First up, Alice and Bob exchange network information. (The expression 'finding candidates' refers to the process of finding network interfaces and ports using the [ICE framework](https://www.html5rocks.com/en/tutorials/webrtc/basics/#ice).)
...
![JSEP architecture diagram](https://www.html5rocks.com/en/tutorials/webrtc/basics/jsep.png)JSEP architecture
...
[![WebRTC architecture diagram](https://www.html5rocks.com/en/tutorials/webrtc/basics/webrtcArchitecture.png)](http://www.webrtc.org/reference/architecture)WebRTC architecture (from [webrtc.org](http://www.webrtc.org/reference/architecture))
...
![Finding connection candidates](https://www.html5rocks.com/en/tutorials/webrtc/basics/stun.png)Finding connection candidates
...
![WebRTC data pathways](https://www.html5rocks.com/en/tutorials/webrtc/basics/dataPathways.png)WebRTC data pathways
...
For signaling, this demo uses the Google App Engine [Channel API](http://code.google.com/appengine/docs/python/channel/overview.html), which enables messaging between JavaScript clients without polling. (WebRTC signaling is covered in more detail [above](https://www.html5rocks.com/en/tutorials/webrtc/basics/#toc-signaling)).

![Architecture of the apprtc video chat application](https://www.html5rocks.com/en/tutorials/webrtc/basics/apprtcArchitecture.png)Architecture of the apprtc video chat application
...
![The Google Channel API: establishing a channel](https://www.html5rocks.com/en/tutorials/webrtc/basics/channelEstablishing.png)The Google Channel API: establishing a channel
...
![The Google Channel API: sending a message](https://www.html5rocks.com/en/tutorials/webrtc/basics/channelSending.png)The Google Channel API: sending a message
...
The underlying purpose is to set up a connection, using a STUN server, with `onIceCandidate()` as the callback (see [above](https://www.html5rocks.com/en/tutorials/webrtc/basics/#stun) for an explanation of ICE, STUN and 'candidate'). Handlers are then set for each of the RTCPeerConnection events: when a session is connecting or open, and when a remote stream is added or removed. In fact, in this example these handlers only log status messages—except for `onRemoteStreamAdded()`, which sets the source for the `remoteVideo` element:
...
The offer creation process here is similar to the no-signaling example [above](https://www.html5rocks.com/en/tutorials/webrtc/basics/#toc-sans) but, in addition, a message is sent to the remote peer, giving a serialized SessionDescription for the offer. This process is handled by `setLocalAndSendMessage():`
...
![Multipoint Control Unit topology diagram](https://www.html5rocks.com/en/tutorials/webrtc/basics/mcu.png)Multipoint Control Unit topology example
...
![Tethr/Tropo demo at Google I/O 2012](https://www.html5rocks.com/en/tutorials/webrtc/basics/tethr.jpg)Tethr/Tropo: disaster communications in a briefcase
...
- WebRTC stats for an ongoing session can be found at:
  ...
  - Example:![chrome://webrtc-internals page](https://www.html5rocks.com/en/tutorials/webrtc/basics/internals.png)chrome://webrtc-internals screenshot
